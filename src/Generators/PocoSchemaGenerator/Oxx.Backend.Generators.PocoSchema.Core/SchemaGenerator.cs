	...
	where TSchemaEventConfiguration : ISchemaEventConfiguration
{
	...
	public async Task CreateFilesAsync()
	{
		...
		_configuration.Events.GenerationCompleted?.Invoke(this, new GenerationCompletedEventArgs(DateTime.Now, generationStartedTime));
	}
	...
	{
		var manuallyGeneratedFiles = fileInfos.Where(x => !x.Name.EndsWith(_configuration.FullFileExtension)).ToArray();
		if (manuallyGeneratedFiles.Length is 0)
		{
			return;
		}

		_configuration.Events.DeletingFilesFailed?.Invoke(this, new DeletingFilesFailedEventArgs(directoryInfo, fileInfos,
			new DirectoryContainsManuallyCreatedFilesException(manuallyGeneratedFiles)));
		Environment.Exit(1);
	}
