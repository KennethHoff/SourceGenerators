...
public class LanguageFileTransformGenerator : ISourceGenerator
{
	...
	private static string ParseSection(XElement element, string fileName, int depth, List<ParsedElement> parsedElements, ParsedElement? parentElement)
	{
		...
		var section = element.Elements().Select(element1 => ParseSection(element1, fileName, nextDepth, parsedElements, newParsedElement)).ToArray();
	...
	private sealed record class ParsedElement(string Name, int Depth, ParsedElement? ParentElement)
	{
